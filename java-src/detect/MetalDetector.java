...
public class MetalDetector {    
    ...
    public MetalDetector(String hostname, int port) {
        this.port = port;
        this.hostname = new String(hostname);
    }

    public MetalDetection detect(BufferedImage bImage) throws IOException{
        Socket socket = null;
        
        MetalDetection detection = null;
        ...
        int counter = 0;
        detection = new MetalDetection(labelSplit);
    ...
    public static void main(String[] args) {
        if(args.length<1){
            ...
            MetalDetector scDetector = new MetalDetector("localhost", 9000);
            try{
                BufferedImage bImage = ImageIO.read(new File(args[0]));
                MetalDetection scDetection = scDetector.detect(bImage);
